...
__all__ = [
    "FedDRServer", "FedDRClient",
    "FedDRServerConfig", "FedDRClientConfig",
]
...
class FedDRServerConfig(ServerConfig):
    ...
    def __init__(self,
                 ...
                 eta:float=1.0,
                 reg_type:str="l1_norm",) -> NoReturn:
        """
        """
        super().__init__(
            "FedDR",
            num_iters, num_clients, clients_sample_ratio,
            eta=eta, reg_type=reg_type,
        )
...
class FedDRClientConfig(ClientConfig):
    ...
    def __init__(self,
                 ...
                 lr:float=1e-3,
                 eta:float=1.0,
                 alpha:float=1.9,) -> NoReturn:
        """
        """
        super().__init__(
            "FedDR", "FedDR",
            batch_size, num_epochs,
            lr, eta=eta, alpha=alpha,
        )
    ...
    def update(self) -> NoReturn:
        ...
        for m in self._received_messages:
            for i, xtp in enumerate(self._x_til_parameters):
                xtp.data.add_(m["x_hat_delta"][i].data, alpha=m["train_samples"]/total_samples)
