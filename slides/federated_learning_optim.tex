...
相关的分布式的模型训练（优化）方法则可以追溯到更早的时间，例如Boyd等人的\cite{boyd2011distributed} \emph{Distributed Optimization and Statistical Learning via the Alternating Direction Method of Multipliers} (2010)
...
Advances and open problems in federated learning (\cite{kairouz2019advances_fl} 2019)给联邦学习下过如下的定义
...
前者一般被称作cross-silo，后者一般被称作cross-device。在cross-device的场景下，一般来说，通信效率才是整个系统的瓶颈所在，此外还需要考虑掉队者（straggler）等问题。
...
Google研究人员McMahan等人在文章\cite{mcmahan2017fed_avg}(2016)中考察了普通的SGD在分布式下的平凡推广FedSGD，即在每次循环中，client执行一次full-batch gradient descent，并做出了进一步推广，提出了FedAvg算法。具体做法就是在每次循环的client local computation中，执行多步mini-batch SGD。这样，既降低了通信开销（communication-efficient），同时也在实验上观察到了模型效果的提升。
...
FedSGD: 在每个client执行一次full-batch GD之后，即进行模型同步（平均）。
...
FedAvg: 每个client执行$K$个mini-batch SGD之后，进行模型同步（平均）。在通信量大大降低的情况下，。。。。（待写）
...
\end{align*}
$x_i$可以视作client $i$上的模型参数，$z$则可被视作server上的模型参数。
...
$\overline{x}$为对所有的client上的模型参数取均值。
...
一些传统的（线性）机器学习模型，当数据在所有client上是垂直划分的（vertical splitting），即不同的client上分布着同一批样本的不同特征，我们还可以把要优化的问题表达为一个Sharing问题。
% Sharing问题就不再赘述举例了
...
所有的client被分为了head group $\mathcal{N}_h$ 与tail group $\mathcal{N}_t$，以此来进行交替方向优化。
