...
$$g_t^{(i)} = \lVert g_t^{(i)} \rVert_{\infty} \cdot \operatorname{sign}(g_t^{(i)}) \circ \framebox{\color{red} $b_t$}$$
where $b_t$ is a random binary vector satisfying some Bernoulli distribution $B(1, \lvert g_{t,k}^{(i)} \rvert / s_t)$
\end{column}
\end{columns}
...
$$Q_s(v) = \lVert v \rVert_2 \cdot \operatorname{sign}(v) \circ \framebox{\color{red}$\xi(v,s)$},$$
where the $i$-th element in vector $\xi(v,s)$ is defined by
\begin{equation*}
    \xi_i(v,s) = \begin{cases}
    \ell/s, & \text{with prob.} 1 - ((|v_i|/\lVert v \rVert_2) s - \ell) \\
...
\begin{equation*}
    \begin{cases}
    \widetilde{g}^{(i)}_t = \widetilde{h}^{(i)}_t + Q(\nabla f_i(x_t) - \widetilde{h}^{(i)}_t) \\
    \widetilde{h}^{(i)}_{t+1} = \widetilde{h}^{(i)}_t + \alpha Q(\nabla f_i(x_t) - \widetilde{h}^{(i)}_t)
    \end{cases}
\end{equation*}
