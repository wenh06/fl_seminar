    ...
    def make_model_rate(self):
        if cfg["model_split_mode"] == "dynamic":
            rate_idx = torch.multinomial(
                torch.tensor(cfg["proportion"]),
                num_samples=cfg["num_users"],
                replacement=True,
            ).tolist()
                        ...
                        if parameter_type == "weight":
                            if v.dim() > 1:
                                ...
                                if "conv1" in k or "conv2" in k:
                                    if idx_i[m] is None:
                                        idx_i[m] = torch.arange(
                                            input_size, device=v.device
                                        )
                                    input_idx_i_m = idx_i[m]
                                    scaler_rate = (
                                        self.model_rate[user_idx[m]]
                            ...
                            if v.dim() > 1:
                                ...
                                if "embedding" in k.split(".")[-2]:
                                    ...
                                    local_input_size = int(
                                        np.ceil(input_size * scaler_rate)
                                    )
                                    input_idx_i_m = torch.arange(
                                        input_size, device=v.device
                                    )[:local_input_size]
                                    ...
                                    output_idx_i_m = (
                                        torch.arange(
                                            output_size, device=v.device
                                        ).reshape(cfg["transformer"]["num_heads"], -1)
                                    )[:, :local_output_size].reshape(-1)
                                    idx_i[m] = output_idx_i_m
                                else:
                                    input_idx_i_m = idx_i[m]
                                    scaler_rate = (
