...
def process_control():
    ...
    if cfg["model_split_mode"] == "dynamic":
        ...
        for i in range(len(mode_rate)):
            cfg["model_rate"] += np.repeat(
                mode_rate[i], num_users_proportion * proportion[i]
            ).tolist()
        cfg["model_rate"] = cfg["model_rate"] + [
            cfg["model_rate"][-1]
            for _ in range(cfg["num_users"] - len(cfg["model_rate"]))
        ]
    else:
...
def make_scheduler(optimizer):
    if cfg["scheduler_name"] == "None":
        ...
        scheduler = optim.lr_scheduler.StepLR(
            optimizer, step_size=cfg["step_size"], gamma=cfg["factor"]
        )
    elif cfg["scheduler_name"] == "MultiStepLR":
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=cfg["milestones"], gamma=cfg["factor"]
        )
    elif cfg["scheduler_name"] == "ExponentialLR":
        scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.99)
    elif cfg["scheduler_name"] == "CosineAnnealingLR":
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=cfg["num_epochs"]["global"], eta_min=cfg["min_lr"]
        )
        ...
        scheduler = optim.lr_scheduler.CyclicLR(
            optimizer, base_lr=cfg["lr"], max_lr=10 * cfg["lr"]
        )
    else:
