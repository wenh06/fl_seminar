...
for i in range(1, len(combination_mode) + 1):
    combination_mode_i = [
        "-".join(list(x)) for x in itertools.combinations(combination_mode, i)
    ]
...
for i in range(1, 10):
    for j in range(len(model_split_mode)):
        for k in range(j + 1, len(model_split_mode)):
            interp += [
                "{}{}-".format(model_split_mode[j], i)
                + "{}{}".format(model_split_mode[k], 10 - i)
            ]
...
def main():
    ...
    save(
        ...
        os.path.join(result_path, "processed_result_history.pt"),
    )
...
def process_result(controls):
    ...
    for control in controls:
        ...
        extract_result(
            list(control), model_tag, processed_result_exp, processed_result_history
        )
        ...
        if os.path.exists(base_result_path_i):
            ...
            for k in base_result["logger"]["test"].mean:
                ...
                if metric_name not in processed_result_exp:
                    processed_result_exp[metric_name] = {
                        "exp": [None for _ in range(num_experiments)]
                    }
                    processed_result_history[metric_name] = {
                        "history": [None for _ in range(num_experiments)]
                    }
                processed_result_exp[metric_name]["exp"][exp_idx] = base_result[
                    "logger"
                ]["test"].mean[k]
                processed_result_history[metric_name]["history"][exp_idx] = base_result[
                    "logger"
                ]["train"].history[k]
        else:
...
def extract_processed_result(extracted_processed_result, processed_result, control):
    if "exp" in processed_result or "history" in processed_result:
        ...
        extracted_processed_result[exp_name][
            "{}_mean".format(metric_name)
        ] = processed_result["mean"]
        extracted_processed_result[exp_name][
            "{}_std".format(metric_name)
        ] = processed_result["std"]
    else:
        for k, v in processed_result.items():
...
def make_df(extracted_processed_result):
    ...
    for exp_name in extracted_processed_result:
        ...
        if data_split_mode == "none":
            ...
            df[df_name].append(
                pd.DataFrame(
                    data=extracted_processed_result[exp_name], index=index_name
                )
            )
        else:
            if model_split_mode == "fix" or control_name == "e1":
                if "-" in control_name:
                    label_name = "-".join(
                        ["{}".format(x[0]) for x in list(control_name.split("-"))]
                    )
                    ...
                    for label_name in interp_name:
                        if control_name[0] == label_name[0]:
                            df_name = "{}_{}".format(
                                "_".join(except_control_name), label_name
                            )
                            df[df_name].append(
                                pd.DataFrame(
                                    data=extracted_processed_result[exp_name],
                                    index=index_name,
                                )
                            )
            else:
...
def make_learning_curve(processed_result):
    ylim_dict = {
        "iid": {
            "global": {"MNIST": [95, 100], "CIFAR10": [50, 100], "WikiText2": [0, 20]}
        },
        "non-iid-2": {
            "global": {"MNIST": [50, 100], "CIFAR10": [0, 70]},
...
def make_stats(model_tag):
    ...
    stats_result_path = os.path.join(
        result_path, "{}_{}_{}.pt".format(data_name, model_name, global_model_mode)
    )
    ...
    for i in range(len(model_mode_list)):
        ...
        stats_result_path_i = os.path.join(
            result_path, "{}_{}_{}.pt".format(data_name, model_name, model_mode_i)
        )
