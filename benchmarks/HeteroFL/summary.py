...
def runExperiment():
    ...
    save_result = total
    save_tag = "{}_{}_{}".format(
        cfg["data_name"], cfg["model_name"], cfg["model_mode"][0]
    )
...
def summarize(data_loader, model):
    def register_hook(module):
        def hook(module, input, output):
            ...
            if key not in summary["module"]:
                ...
                summary["module"][key]["module_name"] = "{}_{}".format(
                    module_name, summary["count"][module_name]
                )
                ...
                if param.requires_grad:
                    if name in ["weight", "in_proj_weight", "out_proj.weight"]:
                        if name not in summary["module"][key]["params"]:
                            ...
                            summary["module"][key]["params"][name]["size"] = list(
                                param.size()
                            )
                            ...
                            summary["module"][key]["params"][name][
                                ...
                                summary["module"][key]["params"][name]["size"],
                                dtype=torch.long,
                                device=cfg["device"],
                            )
    ...
    for key in summary["module"]:
        ...
        for name in summary["module"][key]["params"]:
            num_params += (
                (summary["module"][key]["params"][name]["mask"] > 0).sum().item()
            )
            num_flops += summary["module"][key]["flops"]
        summary["total_num_params"] += num_params
        summary["total_num_flops"] += num_flops
    summary["total_space"] = summary["total_num_params"] * 32.0 / 8 / (1024**2.0)
    return summary
...
def parse_summary(summary):
    ...
    for key in summary["module"]:
        ...
        weight_size = (
            str(summary["module"][key]["params"]["weight"]["size"])
        ...
        records.append(
            [module_name, input_size, weight_size, output_size, num_params, num_flops]
        )
    total_num_param = "{} ({})".format(
        summary["total_num_params"], divide_by_unit(summary["total_num_params"])
    )
    total_num_flops = "{} ({})".format(
        summary["total_num_flops"], divide_by_unit(summary["total_num_flops"])
    )
    total_space = summary["total_space"]
    total = {
        "num_params": summary["total_num_params"],
        "num_flops": summary["total_num_flops"],
