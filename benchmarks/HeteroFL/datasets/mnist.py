...
class EMNIST(MNIST):
    ...
    def make_data(self):
        ...
        classes = {
            "byclass": digits_classes + upper_letters_classes + lower_letters_classes,
            ...
            "letters": upper_letters_classes + unmerged_classes,
            "digits": digits_classes,
        ...
        for subset in subsets:
            train_img[subset] = read_image_file(
                os.path.join(
                    gzip_folder, "emnist-{}-train-images-idx3-ubyte".format(subset)
                )
            )
            train_img[subset] = np.transpose(train_img[subset], [0, 2, 1])
            test_img[subset] = read_image_file(
                os.path.join(
                    gzip_folder, "emnist-{}-test-images-idx3-ubyte".format(subset)
                )
            )
            test_img[subset] = np.transpose(test_img[subset], [0, 2, 1])
            train_target[subset] = read_label_file(
                os.path.join(
                    gzip_folder, "emnist-{}-train-labels-idx1-ubyte".format(subset)
                )
            )
            test_target[subset] = read_label_file(
                os.path.join(
                    gzip_folder, "emnist-{}-test-labels-idx1-ubyte".format(subset)
                )
            )
