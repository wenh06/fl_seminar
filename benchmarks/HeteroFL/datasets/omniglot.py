...
class Omniglot(Dataset):
    ...
    def __getitem__(self, index):
        img, target = Image.open(self.img[index], mode="r").convert("L"), torch.tensor(
            self.target[index]
        )
    ...
    def make_data(self):
        ...
        for i in range(len(train_img)):
            ...
            train_class_i = "/".join(
                os.path.normpath(train_img_i).split(os.path.sep)[-3:-1]
            )
        ...
        for i in range(len(test_img)):
            ...
            test_class_i = "/".join(
                os.path.normpath(test_img_i).split(os.path.sep)[-3:-1]
            )
