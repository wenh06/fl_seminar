...
class ImageNet(Dataset):
    ...
    def __init__(self, root, split, subset, size, transform=None):
        ...
        self.img, self.target = load(
            os.path.join(
                self.processed_folder, str(self.size), "{}.pt".format(self.split)
            )
        )
        self.target = self.target[self.subset]
        self.classes_counts = make_classes_counts(self.target)
        self.classes_to_labels, self.classes_size = load(
            os.path.join(self.processed_folder, str(self.size), "meta.pt")
        )
    ...
    def __getitem__(self, index):
        img, target = Image.open(self.img[index], mode="r").convert(
            "RGB"
    ...
    def make_data(self):
        if not check_exists(os.path.join(self.raw_folder, "base")):
            ...
            extract_file(
                os.path.join(self.raw_folder, "ILSVRC2012_img_train.tar"), train_path
            )
            extract_file(
                os.path.join(self.raw_folder, "ILSVRC2012_img_val.tar"), test_path
            )
            extract_file(
                os.path.join(self.raw_folder, "ILSVRC2012_devkit_t12.tar"), meta_path
            )
            for archive in [
                os.path.join(train_path, archive) for archive in os.listdir(train_path)
            ]:
                extract_file(archive, os.path.splitext(archive)[0], delete=True)
            classes_to_labels, classes_size = make_meta(meta_path)
            with open(
                os.path.join(
                    meta_path, "data", "ILSVRC2012_validation_ground_truth.txt"
                ),
                "r",
            ) as f:
                test_id = f.readlines()
            ...
            for test_id_i in test_id:
                test_node_i = anytree.find_by_attr(
                    classes_to_labels["label"], name="id", value=test_id_i
                )
            ...
            for test_wnid_i, test_img in zip(test_wnid, test_img):
                shutil.move(
                    test_img,
                    os.path.join(test_path, test_wnid_i, os.path.basename(test_img)),
                )
            shutil.move(
                train_path,
            ...
            shutil.move(
                test_path, os.path.join(self.raw_folder, "base", "ILSVRC2012_img_val")
            )
            shutil.move(
                meta_path,
        ...
        train_img, train_label = make_img(
            os.path.join(self.raw_folder, str(self.size), "ILSVRC2012_img_train"),
            classes_to_labels["label"],
        )
        test_img, test_label = make_img(
            os.path.join(self.raw_folder, str(self.size), "ILSVRC2012_img_val"),
            classes_to_labels["label"],
        )
...
def make_meta(path):
    ...
    meta = sio.loadmat(os.path.join(path, "data", "meta.mat"), squeeze_me=True)[
        "synsets"
    ]
...
def make_node(node, branch):
    ...
    for i in range(len(branch)):
        ...
        if (isinstance(branch_children, int) and id == branch_children) or (
            ...
            attribute = {
                "id": parent_attribute["id"] + [id],
                "class": parent_attribute["class"] + [classes],
            }
            break
