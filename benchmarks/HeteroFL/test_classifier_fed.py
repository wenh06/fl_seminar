...
def runExperiment():
    ...
    model = eval(
        'models.{}(model_rate=cfg["global_model_rate"], track=True).to(cfg["device"]).to(cfg["device"])'.format(
            cfg["model_name"]
        )
    )
    ...
    test(
        dataset["test"], data_split["test"], label_split, model, test_logger, last_epoch
    )
    test_logger.safe(False)
    _, _, _, _, _, _, train_logger = resume(
        model, cfg["model_tag"], load_tag="checkpoint", strict=False
    )
    save_result = {
        "cfg": cfg,
        "epoch": last_epoch,
