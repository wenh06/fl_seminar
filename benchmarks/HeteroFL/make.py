...
def main():
    ...
    gpu_ids = [
        ",".join(str(i) for i in list(range(x, x + world_size)))
        for x in list(range(0, num_gpu, world_size))
    ]
    ...
    for i in range(len(control_name)):
        control_names.extend(
            list("_".join(x) for x in itertools.product(*control_name[i]))
        )
    ...
    for i in range(len(controls)):
        ...
        s = (
            s
            + 'CUDA_VISIBLE_DEVICES="{}" python {} --data_name {} --model_name {} --init_seed {} '
