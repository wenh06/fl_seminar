...
for k in cfg:
    exec(
        "parser.add_argument('--{0}', default=cfg['{0}'], type=type(cfg['{0}']))".format(
            k
        )
    )
...
if args["control_name"]:
    cfg["control"] = (
        {k: v for k, v in zip(cfg["control"].keys(), args["control_name"].split("_"))}
...
cfg["metric_name"] = {
    ...
    "test": {
        ...
        "Global": ["Global-Loss", "Global-Accuracy"],
    },
}
...
def runExperiment():
    ...
    model = eval(
        'models.{}(model_rate=cfg["global_model_rate"]).to(cfg["device"])'.format(
            cfg["model_name"]
        )
    )
    ...
    if cfg["resume_mode"] == 1:
        ...
        last_epoch = 1
        data_split, label_split = split_dataset(
            dataset, cfg["num_users"], cfg["data_split_mode"]
        )
        current_time = datetime.datetime.now().strftime("%b%d_%H-%M-%S")
    ...
    for epoch in range(last_epoch, cfg["num_epochs"]["global"] + 1):
        ...
        test(
            dataset["test"], data_split["test"], label_split, test_model, logger, epoch
        )
...
def train(
    dataset, data_split, label_split, federation, global_model, optimizer, logger, epoch
):
    global_model.load_state_dict(federation.global_parameters)
    global_model.train(True)
    local, local_parameters, user_idx, param_idx = make_local(
        dataset, data_split, label_split, federation
    )
    num_active_users = len(local)
    lr = optimizer.param_groups[0]["lr"]
    start_time = time.time()
    for m in range(num_active_users):
        local_parameters[m] = copy.deepcopy(
        ...
        if m % int((num_active_users * cfg["log_interval"]) + 1) == 0:
            ...
            epoch_finished_time = datetime.timedelta(
                seconds=local_time * (num_active_users - m - 1)
            )
            exp_finished_time = epoch_finished_time + datetime.timedelta(
                seconds=round(
                    (cfg["num_epochs"]["global"] - epoch)
                    * local_time
                    * num_active_users
                )
            )
            info = {
                "info": [
                    ...
                    "Train Epoch: {}({:.0f}%)".format(
                        epoch, 100.0 * m / num_active_users
                    ),
...
def stats(dataset, model):
    with torch.no_grad():
        test_model = eval(
            'models.{}(model_rate=cfg["global_model_rate"], track=True).to(cfg["device"])'.format(
                cfg["model_name"]
            )
        )
...
def test(dataset, data_split, label_split, model, logger, epoch):
    with torch.no_grad():
        ...
        for i, input in enumerate(data_loader):
            ...
            evaluation = metric.evaluate(
                cfg["metric_name"]["test"]["Global"], input, output
            )
            logger.append(evaluation, "test", input_size)
        info = {
            "info": [
        ...
        logger.write(
            "test",
            cfg["metric_name"]["test"]["Local"] + cfg["metric_name"]["test"]["Global"],
        )
    return
...
def make_local(dataset, data_split, label_split, federation):
    ...
    user_idx = torch.arange(cfg["num_users"])[
        torch.randperm(cfg["num_users"])[:num_active_users]
    ].tolist()
...
class Local:
    ...
    def train(self, local_parameters, lr, logger):
        metric = Metric()
        model = eval(
            'models.{}(model_rate=self.model_rate).to(cfg["device"])'.format(
                cfg["model_name"]
            )
        )
        ...
        for local_epoch in range(1, cfg["num_epochs"]["local"] + 1):
            for i, input in enumerate(self.data_loader):
                ...
                optimizer.step()
                evaluation = metric.evaluate(
                    cfg["metric_name"]["train"]["Local"], input, output
                )
