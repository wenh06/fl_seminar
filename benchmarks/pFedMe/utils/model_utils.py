...
def get_random_batch_sample(data_x, data_y, batch_size):
    ...
    if len(data_x) > batch_size:
        ...
        if sample_index + batch_size > len(data_x):
            ...
            return (
                data_x[sample_index : sample_index + batch_size],
                data_y[sample_index : sample_index + batch_size],
            )
    else:
        return (data_x, data_y)
...
def read_cifa_data():
    ...
    props = np.random.lognormal(
        0, 2.0, (10, NUM_USERS, NUM_LABELS)
    )  # last 5 is 5 labels
    props = (
        np.array([[[len(v) - NUM_USERS]] for v in cifa_data])
        * props
    ...
    for user in trange(NUM_USERS):
        for j in range(NUM_LABELS):  # 4 labels for each users
            ...
            if idx[l] + num_samples < len(cifa_data[l]):
                ...
                idx[l] += num_samples
                print(
                    "check len os user:", user, j, "len data", len(X[user]), num_samples
                )
...
def read_user_data(index, data, dataset):
    ...
    if dataset == "Mnist":
        ...
        X_train = (
            ...
            .view(-1, NUM_CHANNELS, IMAGE_SIZE, IMAGE_SIZE)
            .type(torch.float32)
        )
        ...
        X_train = (
            torch.Tensor(X_train)
            .view(-1, NUM_CHANNELS_CIFAR, IMAGE_SIZE_CIFAR, IMAGE_SIZE_CIFAR)
            .type(torch.float32)
        )
        y_train = torch.Tensor(y_train).type(torch.int64)
        X_test = (
