...
def devide_train_data(
    ...
    while min_size < min_sample:
        ...
        for l in SRC_CLASSES:
            ...
            if sampling_ratio < 1:
                samples_for_l = min(
                    max_samples_per_user, int(sampling_ratio * len(data[l]))
                )
...
def main():
    ...
    parser.add_argument(
        "--n_class", type=int, default=5, help="number of classification labels"
    )
    parser.add_argument(
        "--random_label",
        type=int,
        default=1,
        help="randomly sampling labels for each task.",
    )
    parser.add_argument(
        "--dirname", type=str, default="", help="Directory name, optional."
    )
    parser.add_argument(
        "--split", type=str, default="letters", choices=["letters", "balanced"]
    )
    parser.add_argument(
    ...
    path_prefix = (
        f"u{args.n_user}-{args.split}-alpha{args.alpha}-ratio{args.sampling_ratio}"
    )
    ...
    train_data, n_train_sample, SRC_N_CLASS = get_dataset(
        mode="train", split=args.split
    )
