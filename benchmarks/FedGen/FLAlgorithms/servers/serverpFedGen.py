...
class FedGen(Server):
    def __init__(self, args, model, seed):
        ...
        self.generative_model = create_generative_model(
            args.dataset, args.algorithm, self.model_name, args.embedding
        )
        if not args.train:
            print(
                "number of generator parameteres: [{}]".format(
                    self.generative_model.get_number_of_parameters()
                )
            )
            print(
                "number of model parameteres: [{}]".format(
        ...
        print(
            "ensemeble alpha = {}, beta = {}, eta = {}".format(
                self.ensemble_alpha, self.ensemble_beta, self.ensemble_eta
            )
        )
        print(
            "generator alpha = {}, beta = {}".format(
                self.generative_alpha, self.generative_beta
            )
        )
        self.init_loss_fn()
        (
        ...
        ) = aggregate_user_data(data, args.dataset, self.ensemble_batch_size)
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
            optimizer=self.optimizer, gamma=0.98
        )

        #### creating users ####
        self.users = []
        for i in range(total_users):
            id, train_data, test_data, label_info = read_user_data(
                i, data, dataset=args.dataset, count_labels=True
            )
    ...
    def train(self, args):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            self.selected_users, self.user_idxs = self.select_users(
                glob_iter, self.num_users, return_idx=True
            )
            if not self.local:
    ...
    def train_generator(
        self, batch_size, epoches=1, latent_layer_idx=-1, verbose=False
    ):
        """
        ...
        def update_generator_(
            n_iters, student_model, TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS
        ):
            ...
            for i in range(n_iters):
                ...
                gen_result = self.generative_model(
                    y_input, latent_layer_idx=latent_layer_idx, verbose=True
                )
                ...
                diversity_loss = self.generative_model.diversity_loss(
                    eps, gen_output
                ...
                for user_idx, user in enumerate(self.selected_users):
                    ...
                    user_result_given_gen = user.model(
                        gen_output, start_layer_idx=latent_layer_idx, logit=True
                    )
                    user_output_logp_ = F.log_softmax(
                        user_result_given_gen["logit"], dim=1
                    )
                    teacher_loss_ = torch.mean(
                        self.generative_model.crossentropy_loss(
                            user_output_logp_, y_input
                        )
                        * torch.tensor(weight, dtype=torch.float32)
                    )
                    teacher_loss += teacher_loss_
                    teacher_logit += user_result_given_gen["logit"] * torch.tensor(
                ...
                student_loss = F.kl_div(
                    F.log_softmax(student_output["logit"], dim=1),
                ...
                TEACHER_LOSS += (
                    self.ensemble_alpha * teacher_loss
                )  # (torch.mean(TEACHER_LOSS.double())).item()
                STUDENT_LOSS += (
                    self.ensemble_beta * student_loss
                )  # (torch.mean(student_loss.double())).item()
                DIVERSITY_LOSS += (
                    self.ensemble_eta * diversity_loss
                )  # (torch.mean(diversity_loss.double())).item()
        ...
        DIVERSITY_LOSS = DIVERSITY_LOSS.detach().numpy() / (
            self.n_teacher_iters * epoches
        )
        info = "Generator: Teacher Loss= {:.4f}, Student Loss= {:.4f}, Diversity Loss = {:.4f}, ".format(
