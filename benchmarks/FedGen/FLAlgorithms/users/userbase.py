...
class User:
    ...
    def __init__(self, args, id, model, train_data, test_data, use_adam=False):
        ...
        if use_adam:
            ...
            self.optimizer = pFedIBOptimizer(
    ...
    def set_parameters(self, model, beta=1):
        for old_param, new_param, local_param in zip(
            ...
            if beta == 1:
                ...
                old_param.data = (
                    beta * new_param.data.clone() + (1 - beta) * old_param.data.clone()
                )
                local_param.data = (
    ...
    def set_prior(self, model):
        for new_param, local_param in zip(
            model.get_encoder() + model.get_decoder(), self.prior_params
        ):
            local_param.data = new_param.data.clone()
    ...
    def set_mask(self, mask_model):
        for new_param, local_param in zip(
            mask_model.get_masks(), self.mask_model.get_masks()
        ):
            local_param.data = new_param.data.clone()
