...
class UserpFedGen(User):
    ...
    def train(
            ...
            for i in range(self.K):
                ...
                if regularization and epoch < early_stop:
                    generative_alpha = self.exp_lr_scheduler(
                        glob_iter, decay=0.98, init_lr=self.generative_alpha
                    )
                    generative_beta = self.exp_lr_scheduler(
                        glob_iter, decay=0.98, init_lr=self.generative_beta
                    )
                    ...
                    logit_given_gen = self.model(
                        gen_output, start_layer_idx=self.latent_layer_idx, logit=True
                    )["logit"]
                    target_p = F.softmax(logit_given_gen, dim=1).clone().detach()
                    user_latent_loss = generative_beta * self.ensemble_loss(
                        user_output_logp, target_p
                    )
        ...
        if personalized:
            self.clone_model_paramenter(
                self.model.parameters(), self.personalized_model_bar
            )
    ...
    def adjust_weights(self, samples):
        ...
        weights = np.array(
            [n_labels / count for count in counts]
