...
class UserpFedEnsemble(User):
    ...
    def update_generator(self, steps, verbose=True):
        ...
        for _ in range(steps):
            ...
            loss, kld_loss, reconstruct_loss = (
                loss_info["loss"],
                loss_info["KLD"],
        ...
        if verbose:
            ...
            info = "VAE-Loss: {:.4f}, KL-Loss: {:.4f}, RC-Loss:{:.4f}".format(
                RECONSTRUCT_LOSS, KLD_LOSS, RC_LOSS
            )
            print(info)
    ...
    def visualize_images(self, generator, glob_iter, repeats=1):
        ...
        path = (
            f"images/{self.algorithm}-{self.dataset}-user{self.id}-iter{glob_iter}.png"
        )
        y = self.available_labels
        y = np.repeat(y, repeats=repeats, axis=0)
        y_input = torch.tensor(y, dtype=torch.int64)
        generator.eval()
        images = generator.sample(y_input, latent=False)[
            "output"
        ]  # 0,1,..,K, 0,1,...,K
