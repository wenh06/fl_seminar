...
def plot_results(args, algorithms):
    ...
    for i, algorithm in enumerate(algorithms):
        ...
        all_curves = np.concatenate(
            [metrics[seed]["glob_acc"] for seed in range(n_seeds)]
        )
        top_accs = np.concatenate(
            [np.sort(metrics[seed]["glob_acc"])[-TOP_N:] for seed in range(n_seeds)]
        )
    ...
    fig_save_path = os.path.join(
        "figs", sub_dir, dataset_[0] + "-" + dataset_[2] + ".png"
    )
