...
def get_data_dir(dataset):
    if "EMnist" in dataset:
        ...
        path_prefix = os.path.join(
            "data", "EMnist", f"u20-{types}-alpha{alpha}-ratio{ratio}"
        )
        ...
        path_prefix = os.path.join(
            "data", "Mnist", "u20c10-alpha{}-ratio{}".format(alpha, ratio)
        )
        train_data_dir = os.path.join(path_prefix, "train")
        test_data_dir = os.path.join(path_prefix, "test")
        ...
        path_prefix = os.path.join(
            "data", "CelebA", "user{}-agg{}".format(user, agg_user)
        )
        train_data_dir = os.path.join(path_prefix, "train")
        test_data_dir = os.path.join(path_prefix, "test")
...
def read_data(dataset):
    ...
    if proxy_data_dir and os.path.exists(proxy_data_dir):
        ...
        proxy_files = [
            f for f in proxy_files if f.endswith(".json") or f.endswith(".pt")
        ]
...
def aggregate_user_test_data(data, dataset_name, batch_size):
    ...
    data_loader, _, unique_labels = aggregate_data_(
        clients, loaded_data, dataset_name, batch_size
    )
...
def aggregate_user_data(data, dataset_name, batch_size):
    ...
    data_loader, data_iter, unique_labels = aggregate_data_(
        clients, loaded_data, dataset_name, batch_size
    )
...
def create_generative_model(dataset, algorithm="", model="cnn", embedding=False):
    ...
    if "FedGen" in algorithm:
        ...
        if "cnn" in algorithm:
            ...
            "-gen" in algorithm
        ):  # we use more lightweight network for sensitivity analysis
            passed_dataset += "-cnn1"
    return Generator(
        passed_dataset, model=model, embedding=embedding, latent_layer_idx=-1
    )
...
def polyak_move(params, target_params, ratio=0.1):
    for param, target_param in zip(params, target_params):
        param.data = param.data - ratio * (
            param.clone().detach().data - target_param.clone().detach().data
        )


def meta_move(params, target_params, ratio):
    for param, target_param in zip(params, target_params):
        target_param.data = param.clone().data + ratio * (
            target_param.clone().data - param.clone().data
        )
...
def update_fast_params(fast_weights, grads, lr, allow_unused=False):
    ...
    for grad, fast_weight in zip(grads, fast_weights):
        if allow_unused and grad is None:
            continue
...
def init_named_params(model, keywords=["encode"]):
    ...
    for name, params in model.named_layers.items():
        if any([key in name for key in keywords]):
            named_params[name] = [
                param.clone().detach().requires_grad_(True) for param in params
            ]
