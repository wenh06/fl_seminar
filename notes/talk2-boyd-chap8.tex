...
\end{align*}
i.e.
$$x_i^{k+1} = (A_i^TA_i + \tilde{\lambda})^{-1} A_i^Tv_i \quad \text{ with $\tilde{\lambda}$ satisfying } \tilde{\lambda}\rho \lVert x_i^{k+1} \rVert_2 = \lambda$$
...
\begin{tikzpicture}
  ...
  \node at (3,-2) {$k = N\rho\left(-\dfrac{1}{N}-s_j\right)$};
  \node[circle,fill,inner sep=2.5pt] (tangent) at (-0.5,2.25) {};
\end{tikzpicture}
\end{figure}

$\overline{z}$-update update splits to the component level, i.e.
